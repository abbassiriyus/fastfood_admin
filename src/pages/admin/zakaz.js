...
export default function ZakazPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [zakazRes, zakazProductsRes, usersRes, productsRes, protsentRes] =
        await Promise.all([
          axios.get(`${url}/zakaz`),
      ...
      if (protsentRes.data.length > 0) {
        setProtsent(protsentRes.data[0].foiz);
      }
  ...
  const getFilteredTotal = () => {
    ...
    filteredZakazlar.forEach((zakaz) => {
      const zakazItems = zakazProducts.filter(
        (zp) => zp.zakaz_id === zakaz.id
      );
  ...
  const filteredZakazlar = zakazlar.filter((zakaz) => {
    ...
    return (
      (filterStol === "" ||
        zakaz.number_stol.toString().includes(filterStol)) &&
  ...
  const currentZakazlar = filteredZakazlar.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
                    ...
                    <td colSpan="6">
                      <div className={styles.accordionContent}>
                        <table className={styles.innerTable}>
                          ...
                          <tfoot>
                            {(() => {
                              const { subtotal, addedPercent, final } =
                                getZakazTotal(zakaz.id);
                              return (
                                <tr>
                                  <td colSpan="4">
                                    <div className={styles.innerTotalBox}>
                                      ...
                                      <p>
                                        Qo‘shilgan foiz ({protsent}%):{" "}
                                        <strong>
                                          {addedPercent.toLocaleString()} so‘m
                                        </strong>
                                      </p>
                                      <p>
                                        Yakuniy summa:{" "}
                ...
                <tr>
                  <td colSpan="6">
                    <div className={styles.totalBox}>
                      ...
                      <p>
                        Qo‘shilgan foiz ({protsent}%):{" "}
                        <strong>{addedPercent.toLocaleString()} so‘m</strong>
                      </p>
                      <p>
                        Umumiy yakuniy summa:{" "}
                        <strong>{final.toLocaleString()} so‘m</strong>
                      </p>
                    </div>
                  </td>
                </tr>
