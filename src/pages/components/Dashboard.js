...
const Dashboard = () => {
  ...
  const myfilter_f = () => {
    axios.get(`${url}/zakaz_products`).then(res => {
      ...
      filtered = filterByDateRange(
        filtered,
        document.querySelector('#start1').value,
        document.querySelector('#end1').value
      );
  ...
  return (
    <div style={{maxWidth:'1300px',margin:'auto'}} className={styles.dashboard}>
      ...
      <div className={styles.filters}>
        ...
        <div className={styles.dateFilter}>
          <label htmlFor="start1">Boshlash:</label>
          <input type="date" id="start1" />
          <label htmlFor="end1">Tugash:</label>
          <input type="date" id="end1" />
          <button onClick={() => myfilter_f()}>Filter</button>
        </div>
      </div>

      {/* Tarix qismi (jadval bilan) */}
